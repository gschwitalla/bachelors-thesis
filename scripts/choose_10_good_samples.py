...
def main(password):
    ...
    # Get cutoff OVRL value (top 10%)
    cutoff_value = np.percentile(df['OVRL'].dropna(), 90)

    # Get samples with OVRL above cutoff
    top_10_percent_df = df[df['OVRL'] >= cutoff_value]

    # Choose 10 random filenames from previous samples
    random_filenames = top_10_percent_df['filename'].sample(n=10, random_state=42)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description= 'Download 10 random LRS3 samples with a good OVRL score (top 10%)')
